...
export default function Signup() {
  ...
  const { isLogged, login, signup }: any = useAuth()
  useEffect(() => {
    if (isLogged) {
      router.push("/")
    }
    ...
    <main className='p-5 py-5'>
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5 p-5 max-w-sm m-auto bg-secondary rounded-md">
          <h1 className='font-bold text-2xl capitalize'>create accunt :) </h1>
          <p className='leading-3'>signup to make your own profile</p>
          {/* single form field */}
          <FormField
          ...
          />
          <Button className='w-full my-5' type="submit">Sign up</Button>
          {/* route */}
