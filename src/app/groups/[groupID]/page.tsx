...
export default function page() {
    ...
    useEffect(() => {
        if (groupID.length > 20 && isLogged && myGroups) {
            let g = myGroups.filter(e => e._id == groupID);
            setgroupDetail(g[0])
    ...
    const tabOptions = [
        {
            value: "Products",
            component: <Products items={groupDetail?.items}/>
        },
        {
            value: "Members",
...
const MiniCard = ({ title = "my funds", coin = 0, icon = <BiRupee /> }) => {
    return (
        <div className='flex flex-col  rounded border p-4 text-base font-semibold transition-colors focus:outline-none border-solid bg-secondary text-secondary-foreground hover:bg-secondary/80 w-[100%] max-w-[15rem] '>
